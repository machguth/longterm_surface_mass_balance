...
def plot_fig_2(outfolder, maxT, minT, pdds, TAa):
    ...
    plt.text(math.ceil(max(pdds[0][4][:]))-((maxT+1)/5), math.ceil(max(pdds[0][6][:])), r'$T_{A}$'+ ': {:} K'.format(TAa[0][0]), fontdict=fonts[0])
    plt.text(math.ceil(max(pdds[1][4][:])), math.ceil(max(pdds[1][6][:])), r'$T_{A}$'+ ': {:} K'.format(TAa[1][0]), fontdict=fonts[0])
...
def plot_fig_3(clim_info, TAa, Tsd, T_zpcl, Tg, p_a, pddf, T_offset, p_offset,
    ...
    for ind, i in enumerate(T_zpcl):
        ...
        if ind > 1:
            ...
            ax1.text(xLeft+xShift+xShift/3, yi-yShift/4,
                 r'B'+str(ind+1)+': $db/dz$: {:.2f} m w.e./100m'.format(dbdz[ind])+r' | ELA: {:} m a.s.l.'.format(int(ELA[ind])),
    ...
    ax1.text(xLeft, yTop - (ind+2.0)*yShift, r'$T_A$ (K): ', fontdict=fonts[2], size=6)
    ax1.text(xLeft, yTop - (ind+2.7)*yShift, r'$\sigma_{daily}$ (K): ', fontdict=fonts[2], size=6)
    ax1.text(xLeft, yTop - (ind+3.4) * yShift, r'$MAAT$ (K) @{:}'.format(z_paleoclim) + ' m a.s.l.:', fontdict=fonts[2], size=6)
    ax1.text(xLeft, yTop - (ind+4.1)*yShift, r'$P$ (m yr$^{-1}$) ' + r'@{:}'.format(z_paleoclim) + ' m a.s.l.:', fontdict=fonts[2], size=6)
    #ax1.text(xLeft, yTop - (ind+4.1)*yShift, r'$P$ (m yr$^{-1}$): ', fontdict=fonts[2], size=6)
    ax1.text(xLeft, yTop - (ind+4.8)*yShift, r'$\gamma_{T}$ (K km$^{-1}$): ', fontdict=fonts[2], size=6)
    ax1.text(xLeft, yTop - (ind+5.5)*yShift, r'$\gamma_{P}$ ((1000 m)$^{-1}$ yr$^{-1}$): ', fontdict=fonts[2], size=6)
    ax1.text(xLeft, yTop - (ind+6.2)*yShift, r'PDDF$_{snow}$ (mm K$^{-1}$ d$^{-1}$): ', fontdict=fonts[2], size=6)
    ax1.text(xLeft, yTop - (ind+6.9)*yShift, r'PDDF$_{ice}$ (mm K$^{-1}$ d$^{-1}$): ', fontdict=fonts[2], size=6)
    ...
    if TAa[0][0] == TAa[1][0]:
        ax1.text(xLeft + dv * xShift, yTop - (ind + 2.0) * yShift, '{:.2f}'.format(TAa[0][0]), fontdict=fonts[2], size=fs)
    else:
        ax1.text(xLeft+dv*xShift, yTop - (ind+2.0)*yShift, '{:.2f}'.format(TAa[0][0])+'; '+'{:.2f}'.format(TAa[1][0]), fontdict=fonts[2], size=fs)
    if Tsd[0] == Tsd[1]:
        ax1.text(xLeft + dv * xShift, yTop - (ind + 2.7) * yShift, '{:}'.format(Tsd[0]), fontdict=fonts[2], size=fs)
    else:
        ax1.text(xLeft+dv*xShift, yTop - (ind+2.7)*yShift, '{:}'.format(Tsd[0])+'; '+'{:}'.format(Tsd[1]), fontdict=fonts[2], size=fs)
    if clim_info[0,0] == clim_info[1,0]:
        ax1.text(xLeft + dv * xShift, yTop - (ind + 3.4) * yShift, '{:.2f}'.format(clim_info[0,0]), fontdict=fonts[2], size=fs)
    else:
        ax1.text(xLeft + dv * xShift, yTop - (ind+3.4) * yShift, '{:.2f}'.format(clim_info[0,0]) + '; ' + '{:.2f}'.format(clim_info[1,0]), fontdict=fonts[2], size=fs)
    if clim_info[0,3] == clim_info[1,3]:
        ax1.text(xLeft + dv * xShift, yTop - (ind + 4.1) * yShift, '{:.3f}'.format(clim_info[0,3]), fontdict=fonts[2], size=fs)
    else:
        ax1.text(xLeft+dv*xShift, yTop - (ind+4.1)*yShift, '{:.3f}'.format(clim_info[0,3])+'; '+'{:.3f}'.format(clim_info[1,3]), fontdict=fonts[2], size=fs)
    if Tg[0] == Tg[1]:
        ax1.text(xLeft+dv*xShift, yTop - (ind+4.8)*yShift, '{:.2f}'.format(Tg[0]*1000), fontdict=fonts[2], size=fs)
    else:
        ax1.text(xLeft + dv * xShift, yTop - (ind + 4.8) * yShift, '{:.2f}'.format(Tg[0] * 1000)+'; '+'{:.2f}'.format(Tg[1]), fontdict=fonts[2], size=fs)
    if p_a[0] == p_a[1]:
        ax1.text(xLeft + dv * xShift, yTop - (ind + 5.5) * yShift, '{:}'.format(p_a[0]), fontdict=fonts[2], size=fs)
    else:
        ax1.text(xLeft+dv*xShift, yTop - (ind+5.5)*yShift, '{:}'.format(p_a[0])+'; '+'{:}'.format(p_a[1]), fontdict=fonts[2], size=fs)
    ax1.text(xLeft+dv*xShift, yTop - (ind+6.2)*yShift, '{:.1f}'.format(pddf[0]), fontdict=fonts[2], size=fs)
    ax1.text(xLeft+dv*xShift, yTop - (ind+6.9)*yShift, '{:.1f}'.format(pddf[2]), fontdict=fonts[2], size=fs)
    ...
    for ind2, i2 in enumerate(T_zpcl): # here add information on specific net balance and AAR
        if ((ind2 + 1) % 2) == 0:
            ...
            colo = 'r'
        ax1.text(xLeft, yTop - (ind + 8+0.7*ind2) * yShift, r'B'+str(ind2+1)+' (m w.e.): ', fontdict=fonts[2], color=colo, size=fs)
        ax1.text(xLeft+dv*xShift, yTop - (ind + 8+0.7*ind2)*yShift, '{:.3f}'.format(B_glw[ind2]), fontdict=fonts[2], color=colo, size=fs)
        ax1.text(xLeft, yTop - (ind + 8+0.7*(ind2+len(T_zpcl))) * yShift, r'AAR'+str(ind2+1)+' (-): ', fontdict=fonts[2], color=colo, size=fs)
        ax1.text(xLeft+dv*xShift, yTop - (ind + 8+0.7*(ind2+len(T_zpcl)))*yShift, '{:.3f}'.format(AAR[ind2]), fontdict=fonts[2], color=colo, size=fs)
